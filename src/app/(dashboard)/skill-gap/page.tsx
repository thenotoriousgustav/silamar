            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded-none border border-white/10 bg-surface-800 px-4 py-2.5 text-sm text-white placeholder-surface-300 outline-none focus:border-brand-500"
                />
                <button
                  type="button"
                  onClick={addSkill}
                  className="rounded-none bg-surface-700 px-3 py-2.5 text-surface-300 hover:bg-surface-600 hover:text-white"
                >
              ...
              {skills.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {skills.map((skill) => (
                    <span
                      key={skill}
                      className="flex items-center gap-1 rounded-none bg-brand-500/10 px-3 py-1 text-xs font-medium text-brand-400"
                    >
                      {skill}
          ...
          {result && (
            <>
              ...
              <div className="glass rounded-none p-6">
                ...
                <div className="mb-3 h-2 overflow-hidden rounded-none bg-surface-700">
                  <div
                ...
                <div className="glass rounded-none p-6">
                  ...
                  <div className="space-y-3">
                    {result.missingSkills.map((skill, i) => (
                      <div key={i} className={`rounded-none border p-3 ${PRIORITY_COLORS[skill.priority]}`}>
                ...
                <ol className="space-y-2">
                  {result.learningPath.map((step, i) => (
                    <li key={i} className="flex items-start gap-3 text-sm text-surface-300">
                      <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-none bg-brand-600/20 text-xs font-bold text-brand-400">
                        {i + 1}
                      </span>
                      {step}
                    </li>
                ...
                <div className="glass rounded-none p-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {result.matchedSkills.map((skill) => (
                      <span key={skill} className="rounded-none bg-emerald-500/10 px-3 py-1 text-xs font-medium text-emerald-400">
                        {skill}
                      </span>
                    ))}
                  </div>
