...
/**
 * CoverLetterPdfPreview — 100% flicker-free PDF preview showing ALL pages.
 *
 ...
 *
 * This mirrors the resume builder's pdf-preview.tsx, adapted for cover letters
 * (CoverLetterTemplate + Partial<CoverLetterBuilderData>).
 */
...
export function PdfPreview({ content }: PdfPreviewProps) {
    ...
    const generate = async () => {
      ...
      try {
        const blob = await pdf(
          <CoverLetterTemplate data={debouncedContent} />,
        ).toBlob();
  ...
  // ── Download ──────────────────────────────────────────────────────────
  const pdfFileName = `${
    content.fullName?.replace(/\s+/g, "_") || "surat"
  }_lamaran.pdf`;
