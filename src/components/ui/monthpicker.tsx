...
function MonthCal({
  ...
  const initialDate =
    selectedMonth && !isNaN(selectedMonth.getTime())
      ? selectedMonth
      : new Date();
              ...
              <tr key={"row-" + a} className="mt-2 flex w-full">
                {monthRow.map((m) => {
                  return (
                    <td
                      key={m.number}
                      className="[&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected])]:bg-accent relative h-10 w-1/4 p-0 text-center text-sm focus-within:relative focus-within:z-20 first:[&:has([aria-selected])]:rounded-none last:[&:has([aria-selected])]:rounded-none [&:has([aria-selected].day-range-end)]:rounded-none"
                    >
                      <button
                        onClick={() => {
                          setMonth(m.number);
